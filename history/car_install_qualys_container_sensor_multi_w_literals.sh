...
LOCATION="https://qualyscontainersensor.blob.core.windows.net/qualyssensorinstaller/QualysContainerSensor.tar.xz?se=2025-06-20T23%3A59%3A00Z&sp=r&spr=https&sv=2022-11-02&sr=b&sig=g3S9IMYtB27NVeUyykZlsWMMkG1cKAea%2F%2FBL5OLSSWU%3D"
...
POD_URL=""
#POD_URL="$4" # Qualys POD URL, required when Location == Dockerhub
#POD_URL="https://cmsqagpublic.qg1.apps.qualys.co.uk/ContainerSensor"

# Add any install script options here
#INSTALL_OPTIONS="$5" # Additional sensor install options, e.g. --registry-sensor
#INSTALL_OPTIONS="--registry-sensor --perform-sca-scan --perform-malware-detection"
INSTALL_OPTIONS="" # Default to empty if not set

# Force a reinstall if existing sensor found
FORCE_REINSTALL="true" # Force a reinstall if the sensor is already deployed true|false
...
install_from_dockerhub() {
  ...
  CONTAINER_ID=$(sudo $RUNTIME run -d --restart on-failure \
    ...
    --perform-sca-scan $INSTALL_OPTIONS 2>&1)
